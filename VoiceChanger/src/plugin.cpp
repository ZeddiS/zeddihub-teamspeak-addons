...
VoicePreset menuToPreset(int menuItemID) {
    switch (menuItemID) {
        ...
        case MENU_ID_PRESET_BYPASS:     return VoicePreset::Bypass;
...
const char* presetName(VoicePreset p) {
    switch (p) {
        ...
        case VoicePreset::Bypass:     return "Bypass";
        default:                      return "?";
    }
}
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
    ...
    constexpr Spec items[] = {
        ...
        { MENU_ID_PRESET_BYPASS,     "VC: Bypass (diagnostic)" },
...
void ts3plugin_onMenuItemEvent(uint64 schid,
    ...
    if (menuItemID >= MENU_ID_PRESET_OFF && menuItemID <= MENU_ID_PRESET_BYPASS) {
        VoiceConfig c = g_engine->config();
