...
bool VoiceEngine::processSamples(short* samples, int sampleCount, int channels) {
    ...
    switch (cfg.preset) {
        ...
        case VoicePreset::VolumeBoost: {
            // Sanity test -- just 1.5x gain.
            for (int i = 0; i < totalSamples; ++i) {
                floatScratchOut_[i] *= 1.5f;
            }
            break;
        }
        case VoicePreset::Bypass: {
            // Diagnostic: NO modification. With compat mode OFF, edited=1
            // gets set anyway -- tests if the flag itself breaks user's VAD.
            break;
        }
