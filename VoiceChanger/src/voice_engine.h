...
enum class VoicePreset : int {
    ...
    Bypass     = 14,  // DIAGNOSTIC -- pure passthrough, sets edited=1 to test if flag breaks VAD
};
...
struct VoiceConfig {
    ...
    // callback. Keeps TS3 voice activity detection working. Default ON
    // since most users report VAD breaks when edited=1 is set.
